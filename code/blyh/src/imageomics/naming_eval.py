...
def to_classes(data,text_type):
    ...
    if text_type == 'sci':
        ...
        import inspect
        # 获取 Taxon 类构造函数实际支持的参数列表
        taxon_params = inspect.signature(Taxon.__init__).parameters.keys()

        def create_taxon(row):
            row_dict = row.to_dict()
            # 只保留 Taxon 类支持的参数，过滤掉多余的（如 scientific_name）
            filtered_dict = {k: v for k, v in row_dict.items() if k in taxon_params}
            return Taxon(**filtered_dict).taxon_common_name

        return data_view.apply(create_taxon, axis=1)
    else:
